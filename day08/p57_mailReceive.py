...
slack_url = 'https://hooks.slack.com/services/T**/B0**'

def sendToSlack(msg):
    header = { 'Content-ytpe' : 'application/json'}
    data = { 'text' : msg}
...
if res[0] == 'OK':
    ...
    for mail in lastEmails:
        ...
        # subject, endcode = find_encoding_info(emailMessage['Subject'])
        # print(f'제목 : {subject}')
        # print('내용 : ')
        # msg = ''
        ...
        #             endcode = part.get_content_charest()
        #             msg = msg + str(bytes, endcode) # 인코딩을 자신의 언어에맞춰서 변경
